...
class _EditState extends State<Edit> {
  ...
  Future _getData() async {
    try {
      final response = await http.get(Uri.parse(
          ...
          "http://192.168.10.5/backend/detail.php?id='${widget.id}'"));
  ...
  Future _onUpdate(context) async {
    try {
      return await http.post(
        Uri.parse("http://192.168.10.5/backend/update.php"),
        body: {
          "id": widget.id,
          "title": title.text,
  ...
  Future _onDelete(context) async {
    try {
      return await http.post(
        Uri.parse("http://192.168.10.5/backend/detele.php"),
        body: {
          "id": widget.id,
        },
      ).then((value) {
                ...
                onPressed: () {
                  showDialog(
                    ...
                    builder: (BuildContext context) {
                      ...
                      return AlertDialog(
                        content: Text('Yakin ingin menghapus note ini?'),
                        actions: <Widget>[
                          ElevatedButton(
